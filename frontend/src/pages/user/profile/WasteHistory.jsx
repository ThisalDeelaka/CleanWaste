      ...
      <table className="min-w-full bg-white shadow-md rounded-lg">
        <thead>
          <tr className="bg-gray-200 text-gray-600 text-left text-xs md:text-base">
            <th className="py-2 px-2 md:px-4">Waste Type</th>
            <th className="py-2 px-2 md:px-4">Location</th>
          ...
          {wasteRequests.length > 0 ? (
            wasteRequests.map((request) => (
              <tr key={request._id} className="border-t">
                <td className="py-2 px-2 md:px-4">{request.wasteType}</td>
                <td className="py-2 px-2 md:px-4">
                  {request.location.latitude}, {request.location.longitude}
                </td>
                <td className={`py-2 px-2 md:px-4 ${request.status === 'pending' ? 'text-red-500' : 'text-green-500'}`}>
                  {request.status}
                </td>
                <td className="py-2 px-2 md:px-4">{new Date(request.createdAt).toLocaleDateString()}</td>
              </tr>
            ))
          ) : (
            <tr>
